...
import { Plus, Search, Archive, Pencil, Trash2, Flame, Trophy, ArchiveRestore, Sparkles } from "lucide-react";
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          <h1 className="text-2xl md:text-3xl font-semibold tracking-tight">All habits</h1>
          <p className="text-sm text-muted mt-0.5">Manage every habit you've ever created.</p>
        </div>
      ...
      <div className="card p-4">
        <div className="flex flex-col md:flex-row gap-3 md:items-center">
          ...
          <div className="inline-flex rounded-xl glass overflow-hidden text-sm">
            <button onClick={() => setShowArchived(false)} className={`px-3.5 py-2.5 font-medium transition ${!showArchived ? "bg-brand-500/15 text-brand-700 dark:text-brand-300" : "text-soft hover:bg-[var(--surface-hover)]"}`}>
              Active · {activeCount}
            </button>

            <button onClick={() => setShowArchived(true)} className={`px-3.5 py-2.5 font-medium transition border-l divider ${showArchived ? "bg-brand-500/15 text-brand-700 dark:text-brand-300" : "text-soft hover:bg-[var(--surface-hover)]"}`}>
      ...
      {filtered.length === 0 ? (
        <div className="card p-10 text-center">
          ...
          <div className="font-medium">
            {showArchived ? "Nothing archived" : habits.length === 0 ? "No habits yet" : "No habits match your filter"}
          </div>
        ...
        <div className="space-y-2">
          {filtered.map((h) => {
            ...
            return (
              <div key={h._id} className={`card p-4 flex items-center gap-4 ${h.isArchived ? "opacity-70" : ""}`}>
                <div className="w-11 h-11 rounded-xl flex items-center justify-center text-xl shrink-0" style={{ background: `${h.color}26`, color: h.color }}>
                  {h.icon}
                </div>
                ...
                <div className="hidden sm:flex items-center gap-4 text-sm">
                  <div className="flex items-center gap-1" title="Current streak">
                    <Flame size={14} className={current > 0 ? "text-orange-500" : "text-faint"} />
                ...
                <div className="flex items-center gap-1">
                  <button className="btn-ghost p-2" onClick={() => { setEditing(h); setFormOpen(true); }} title="Edit">
                    <Pencil size={16} />
                  </button>

                  <button className="btn-ghost p-2" onClick={() => archive(h)} title={h.isArchived ? "Unarchive" : "Archive"}>
                    {h.isArchived ? <ArchiveRestore size={16} /> : <Archive size={16} />}
                  </button>

                  <button className="btn-ghost p-2 text-rose-500 hover:bg-rose-500/10 hover:text-rose-400" onClick={() => setDeleteTarget(h)} title="Delete">
                    <Trash2 size={16} />
                  </button>
                </div>
              </div>
      ...
      <Modal open={formOpen} onClose={() => { setFormOpen(false); setEditing(null); }} title={editing ? "Edit habit" : "New habit"}>
        <HabitForm initial={editing} submitting={submitting} onCancel={() => { setFormOpen(false); setEditing(null); }} onSubmit={save} />
      </Modal>
      ...
      <Modal open={!!deleteTarget} onClose={() => setDeleteTarget(null)} title="Delete habit?" maxWidth="max-w-sm">
        ...
        <div className="flex justify-end gap-2 mt-5">
          ...
          <button className="inline-flex items-center justify-center gap-2 rounded-xl bg-gradient-to-br from-rose-500 to-red-600 px-4 py-2.5 text-sm font-medium text-white hover:brightness-110 shadow-lg shadow-black/15 transition" onClick={() => remove(deleteTarget)}>
            Delete
          </button>
        </div>
      </Modal>
