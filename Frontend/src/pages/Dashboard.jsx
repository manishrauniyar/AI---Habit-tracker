...
export default function Dashboard() {
  ...
  const loadAll = async () => {
    ...
    try {
      ...
      const allRange = await api.get("/logs/range", { params: { start: s90, end: e90 } });
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      {recoveryHabit && (
        <StreakRecoveryCard habit={recoveryHabit} onDismiss={() => {
          const dismissed = JSON.parse(localStorage.getItem("recovery-dismissed") || "{}");
      ...
      <div className="card p-5">
        ...
        {habits.length === 0 ? (
          ...
          <div className="space-y-2">
            {habits.map((h) => (
              <TodayHabitCard key={h._id} habit={h} completed={completedToday.has(String(h._id))} streak={streaksById[h._id]?.current || 0} onToggle={() => toggle(h)} onEdit={() => { setEditing(h); setFormOpen(true); }} onArchive={() => archiveHabit(h)} onDelete={() => setDeleteTarget(h)} />
            ))}
          </div>
        )}
      </div>
      ...
      <Modal open={formOpen} onClose={() => { setFormOpen(false); setEditing(null); }} title={editing ? "Edit habit" : "New habit"}>
        <HabitForm initial={editing} submitting={submitting} onCancel={() => { setFormOpen(false); setEditing(null); }} onSubmit={saveHabit} />
      </Modal>
      ...
      <Modal open={!!deleteTarget} onClose={() => setDeleteTarget(null)} title="Delete habit?" maxWidth="max-w-sm">
        ...
        <div className="flex justify-end gap-2 mt-5">
          ...
          <button className="inline-flex items-center justify-center gap-2 rounded-xl bg-gradient-to-br from-rose-500 to-red-600 px-4 py-2.5 text-sm font-medium text-white hover:brightness-110 shadow-lg shadow-black/15 transition" onClick={() => deleteHabit(deleteTarget)}>
            Delete
          </button>
        </div>
      </Modal>
