    ...
    <aside className="hidden md:flex md:flex-col w-64 fixed inset-y-0 left-0 z-30 glass border-r sidebar-shell">
      <div className="px-6 py-5 border-b divider">
        <div className="flex items-center gap-2">
          <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-ink-900 to-brand-700 text-white flex items-center justify-center brand-mark">
            <img src={habitLogo} alt="Habit Tracker" className="w-full h-full object-contain" />
          </div>
      ...
      <nav className="flex-1 px-3 py-4 space-y-1">
        {nav.map(({ to, label, icon: Icon }) => (
          <NavLink key={to} to={to} className={({ isActive }) => `nav-item flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition ${isActive ? "nav-item-active bg-gradient-to-r from-brand-500/15 via-brand-300/10 to-transparent text-brand-700 dark:text-brand-300 ring-1 ring-brand-500/20" : "text-soft hover:bg-[var(--surface-hover)]"}`}>
            <Icon size={18} />
            {label}
          </NavLink>
        ))}
      ...
      <div className="p-3 border-t divider space-y-1">
        ...
        <button className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm text-soft hover:bg-[var(--surface-hover)] transition" onClick={() => setSettingsOpen(true)}>
          <Settings size={18} />
          Settings
        </button>
        ...
        <div className="px-2 py-2 flex items-center gap-3">
          ...
          <button onClick={logout} title="Log out" className="p-2 rounded-lg text-soft hover:bg-[var(--surface-hover)]">
            <LogOut size={16} />
          </button>
        </div>
      </div>
          ...
          <label className="flex items-start gap-3 p-3 rounded-xl glass cursor-pointer hover:bg-[var(--surface-hover)]">
            ...
            <div>
              ...
              <div className="text-xs text-faint">
                Show a short personalised AI message every morning on the dashboard.
              </div>
            </div>
          </label>
