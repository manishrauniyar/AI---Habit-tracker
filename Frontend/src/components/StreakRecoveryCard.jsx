...
export default function StreakRecoveryCard({ habit, onDismiss }) {
  ...
  return (
    <div className="relative rounded-2xl p-5 glass overflow-hidden animate-slide-up">
      <div className="absolute inset-0 pointer-events-none opacity-60" style={{ background: "radial-gradient(circle at 0% 0%, rgba(176,95,73,0.16), transparent 55%), radial-gradient(circle at 100% 100%, rgba(202,167,120,0.16), transparent 55%)" }} />
      ...
      <div className="flex items-start gap-3 pr-6 relative">
        <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-brand-500 to-brand-700 text-white flex items-center justify-center shrink-0 shadow-lg shadow-black/15">
          <Heart size={18} />
        </div>
        ...
        <div className="flex-1">
          ...
          {!content ? (
            <button className="mt-3 btn-primary" onClick={generate} disabled={loading}>
              {loading ? (
                <>
