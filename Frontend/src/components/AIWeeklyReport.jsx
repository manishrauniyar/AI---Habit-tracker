...
export default function AIWeeklyReport() {
  ...
  return (
    <div className="card p-5 relative overflow-hidden">
      <div className="absolute inset-0 pointer-events-none opacity-40" style={{ background: "radial-gradient(circle at 0% 0%, rgba(202,167,120,0.24), transparent 60%)" }} />

      <button onClick={() => setExpanded((e) => !e)} className="w-full flex items-center gap-3 text-left relative">
        <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-ink-900 to-brand-700 text-white flex items-center justify-center shrink-0 shadow-lg shadow-black/15">
          <Sparkles size={18} />
        </div>
        ...
        <ChevronDown size={18} className={`text-faint transition ${expanded ? "rotate-180" : ""}`} />
      </button>
